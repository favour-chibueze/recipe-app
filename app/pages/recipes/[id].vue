...
const {data, error} = useFetch<Recipe>(`https://dummyjson.com/recipes/${params.id}`);
...
useSeoMeta({
  ...
  description: 'Recipes for you to try at home!',
  ogTitle: data.value?.name,
  ogDescription: 'Recipes for you to try at home!',
  ogImage: data.value?.image,
  ogUrl: `http:localhost:3000/recipes/${data.value?.id}`,
  twitterTitle: data.value?.name,
  twitterDescription: 'Recipes for you to try at home! ',
